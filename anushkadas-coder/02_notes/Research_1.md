...
* **Architecture & Methodology:**
  ...
  * **Evaluation Metric:** Evaluates predictive models using strict spatial metrics such as Mean Absolute Error (MAE) and Mean Squared Error (MSE), defined as $\text{MSE} = \frac{1}{M} \sum_{j=1}^{M} (C_j - \hat{C}_j)^2$, where $C_j$ is the actual crowd count in a frame and $\hat{C}_j$ is the predicted count.
